...
void NhanVien::nhapNV(){
	...
	while (true) {
		cout << "Nhap luong trung binh(VND): ";
		string luongInput; // Khai báo biến để lưu giá trị nhập vào
		getline(cin, luongInput);
		bool isValid = true;
    	for (char c : luongInput) { // Duyệt qua từng ký tự trong chuỗi lương nhập vào
			if (!isdigit(c) && c != '.' && c != '-') { 
				isValid = false;
				break;
			}
		...
		if (isValid) {//nếu hợp lệ
			luong = stod(luongInput); // Chuyển đổi chuỗi nhập vào thành số thực
			if (luong < 0) { 
				cout << "\033[31m"; // red
				cout << "Nhap sai! Vui long nhap lai luong." << endl;
				cout << "\033[0m"; // reset
			} else {
				break; 
			}
...
void NhanVien::xuatNV(){
    cout << "|" << left<< setw(12) << id 
		        ...
		        << "|" << setw(40) << diaChi
				<< "|" << setw(28) << chucVu  
				<< "|" << setw(26) << fixed << setprecision(2) << luong << "|" << endl;
}
...
void NhanVien::xuatDanhSachNV( NhanVien ds[],int n){
    cout << "\n+------------+----------------------+-----------+-----------+---------------+------------+---------------------------+----------------------------------------+----------------------------+--------------------------+\n";
    cout << "|    ID      |        Ho ten        | Gioi tinh | Ngay Sinh |     CCCD      |     SDT    |          Email            |                 Dia chi                |          Chuc Vu           |     Luong Co Ban(VND)    |\n";
    cout << "+------------+----------------------+-----------+-----------+---------------+------------+---------------------------+----------------------------------------+----------------------------+--------------------------+\n";
    for(int i=0;i<n;i++){
    	ds[i].xuatNV();
    cout << "+------------+----------------------+-----------+-----------+---------------+------------+---------------------------+----------------------------------------+----------------------------+--------------------------+\n";
